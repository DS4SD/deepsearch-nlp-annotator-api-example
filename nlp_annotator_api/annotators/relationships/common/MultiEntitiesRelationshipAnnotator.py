...
from .BaseTextRelationshipAnnotator import BaseTextRelationshipAnnotator
...
class MultiEntitiesRelationshipAnnotator(BaseTextRelationshipAnnotator):
    ...
    def columns(self) -> list:
        return list([{"key": entity, "entities": [entity]} for entity in self._entities])
    ...
    def annotate_relationships_text(self, text: str, entity_map: dict) -> dict:
        ...
        for entity_name in self._entities:
            if entity_name in entity_map and len(entity_map[entity_name]) > 0:
                matches.append([f'{entity_name}.{i}' for i, _ in enumerate(entity_map[entity_name])])
